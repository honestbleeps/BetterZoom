...
BetterZoom is intended to be an improvement on the very popular HoverZoom
extension, but without the shady marketing / data collection going on.

There's lots of work to be done in terms of adding functionality, specific
site support, etc - but I feel this is a good start.

In addition, by using BabelExt as a boilerplate, BetterZoom will be
cross-browser compatible with: Chrome, Firefox, Opera, Safari.

At the time of this writing, BetterZoom does not yet use any of BabelExt's
APIs, but it is built off of the BabelExt boilerplate.

Note: You'll need to run makelinks.sh (linux, osx) or makelinks.bat (windows)
to copy or link necessary files into each browser's build directory.

Below are the Readme.md notes from BabelExt for an explanation of what it is.
