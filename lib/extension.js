...
var BetterZoom = {
  ...
  getOptimalImagePosition: function() {
    var maxWidth, maxHeight, top, left,
        offsetX = 20, // how far away horizintally from the mouse, in pixels?
    ...
    if (BetterZoom.mouse.x < screenCenterX) {
      ...
      maxWidth = screenWidth - BetterZoom.mouse.x - offsetX - screenPadding;
    } else {
      // if right of center:
      ...
      maxWidth = BetterZoom.mouse.x - offsetX - screenPadding;
    }
  
    top = screenTop + screenPadding;
    ...
    maxHeight = screenHeight - screenPadding - headerHeight;  
/*
    if (BetterZoom.mouse.y < screenCenterY) {
      ...
      maxHeight = screenBottom - (BetterZoom.mouse.y + offsetY) - screenPadding;
    } else {
      // if below center:
      ...
      maxHeight = screenTop - BetterZoom.mouse.y - offsetY - screenPadding;
    }
*/
    return {
      position: {
  ...
  showMedia: function(mediaData) {
    ...
    if (mediaData.type === 'img') {
      ...
      $(BetterZoom.container)
        .css({
          top: BetterZoom.mouse.y,
          left: BetterZoom.mouse.x + 20 // width of callout
        })
        .show();
