...
var BetterZoom = {
  ...
  init: function() {
    ...
    this.header = 
      $('<div class="BetterZoom-header"> ' +
          '<span>BetterZoom</span>' +
          '<div id="BetterZoom-toolbar"></div>' +
        '</div>');
  ...
  getOptimalImagePosition: function() {
    var maxWidth, maxHeight, top, left, right, rel,
    ...
    if (BetterZoom.mouse.x < screenCenterX) {
      ...
      rel = 'left';
      left = null;
      right = (screenWidth - BetterZoom.mouse.x) + offsetX - BetterZoom.screenPadding;
  ...
  showMedia: function(mediaData) {
    ...
    var screenTop = $(window).scrollTop(),
        optimalPos = BetterZoom.getOptimalImagePosition(),
        calloutPos;
    ...
    if (optimalPos.relativeToCenter === 'right') {
      ...
      calloutPos.left = BetterZoom.mouse.x + 10;
      calloutPos.right = null;
    } else {
      // add a bit of padding to allow clicks to still work.
      calloutPos.left = null;
      calloutPos.right = $(window).width() - BetterZoom.mouse.x - 10;
    }

    $(this.callout)
      .css(calloutPos)
      .show()
      .attr('class','BetterZoom-callout ' + optimalPos.relativeToCenter);

    $(BetterZoom.mediaContainer).css(optimalPos.size);
    ...
    if (mediaData.type === 'img') {
      ...
      $(BetterZoom.img).css(optimalPos.size);
