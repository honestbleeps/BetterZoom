...
BZ = BetterZoom = {
  ...
  init: function() {
    ...
    this.video = $('<video autoplay loop muted></video>');
    this.audio = $('<audio>');

    this.videoSource = $('<source>');

    $(this.video)
      .append(this.videoSource);
  ...
  showMedia: function(mediaData) {
    ...
    if (mediaData.type === 'img') {
      ...
      BZ.populateVideo(optimalPos, mediaData.src, mediaData.title);
  ...
  populateVideo: function(optimalPos, src, title) {
    // empty out the mediaContainer and place in our loader and image.
    $(BZ.mediaContainer)
      .empty()
      .append(BZ.video);
