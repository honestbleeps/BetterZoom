...
BZ = BetterZoom = {
  ...
  showMedia: function(mediaData) {
    ...
    if (mediaData.type === 'img') {
      ...
      BZ.showingAlbum = true;
      BZ.currentAlbum = mediaData.images;
      BZ.albumOptimalPos = optimalPos;

      var firstImage = mediaData.images[0].src;
      BZ.currentAlbumIndex = 0;
      
      if(BZ.albumBookmarks[firstImage] != undefined)
        BZ.currentAlbumIndex = BZ.albumBookmarks[firstImage];
      console.log(BZ.albumBookmarks);

      BZ.populateImage(optimalPos, mediaData.images[0].src, mediaData.images[0].title);
      BZ.albumInfo.text((BZ.currentAlbumIndex+1)+"/"+BZ.currentAlbum.length);
    }
    return true;
  },
  changeAlbumImage: function(val){
    var currentIndex = (BZ.currentAlbumIndex + val)%BZ.currentAlbum.length,
        length = BZ.currentAlbum.length;

    if(currentIndex < 0) currentIndex = length -1;
    BZ.albumInfo.text((currentIndex+1)+"/"+length);

    
    BZ.albumBookmarks[BZ.currentAlbum[0].src] = BZ.currentAlbumIndex = currentIndex; 
    BZ.populateImage(BZ.albumOptimalPos, BZ.currentAlbum[currentIndex].src, BZ.currentAlbum[currentIndex].title);
  },
  ...
  handleKeyPress: function(e) {
    ...
    switch (e.keyCode) {
      ...
      case 84: //t
        ...
        break;
      case 87: //w
        window.open(BZ.img.src, ' ', directories=0);
        break;
      default:
        break;
    }
