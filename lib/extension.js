...
BZ = BetterZoom = {
  ...
  showMedia: function(mediaData) {
    ...
    if (mediaData.link && mediaData.link.href) {
      // mark the link as visited. the callback needs to exist, but nothing needs to be done.
      BabelExt.history.add(mediaData.link.href, function() {});
    }

    if (mediaData.type === 'img') {
      BZ.populateImage(optimalPos, mediaData.src, mediaData.title);
    } else if (mediaData.type === 'video') {
      BZ.populateVideo(mediaData.src, mediaData.title, mediaData.width, mediaData.height, mediaData.loop, mediaData.mute, mediaData.sources);
  ...
  populateVideo: function(src, title, width, height, loop, mute, sources) {
    // empty out the video tag, set the optimal size for the video
    // note 
    $(BZ.video)
      .empty();
    ...
    if(mute === true) {
        ...
        $(BZ.video).removeAttr("muted");
    }

    // empty out the mediaContainer and place our video
    $(BZ.mediaContainer)
      .empty()
      .append(BZ.video);
    ...
    // mark as visited. the callback needs to exist, but nothing needs to be done.
    BabelExt.history.add(src, function() {});

    // create and add the source tags for each video source
    for (var i = 0, l = sources.length; i < l; i++) {
        var source = sources[i];

        var sourceTag = $('<source>');

        // change src to new URL we just received
        sourceTag
          .attr('src', source.src)
    ...
    // helps getOptimalImagePosition calculate properly
    BZ.img.height = height;

    // position, size, and show the container
    var pos = BZ.getOptimalImagePosition(width, height);
    $(BZ.container)
      .css(
        pos.position
      )
  ...
  staticImage: {
    checkLink: function(link) {
      ...
      if(link && link.innerText && link.innerText.length > 0)
      {
